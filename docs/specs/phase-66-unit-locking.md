...
- [ ] **Model + migration** — add `is_locked = models.BooleanField(default=False, db_default=False)`
      ...
      on it.
- [ ] **Enforcement helper** — `require_unit_unlocked(user, unit)` in
...
- [ ] **Gate lesson-level reads** (`courses/views.py`) — call the helper (via
...
- [ ] **Gate quiz reads/writes** (`quizzes/views.py`) — unit quiz list (:43), quiz
...
- [ ] **Serializer shaping** — expose `is_locked` on `UnitSerializer`
...
- [ ] **Toggle write path** — make `is_locked` writable on `UnitCreateSerializer`
...
- [ ] **Progress denominators** — exclude lessons of locked units (for the student in
...
- [ ] **Course map** — `course_map` (views.py:3144): every node in a locked unit gets
...
- [ ] **Dashboard continue-learning** — the next-lesson picker (views.py:682-683)
      skips lessons in locked units.
- [ ] **Seed safety (verify, no code expected)** — `upsert_unit`
...
- [ ] **Tests** (`courses/`, `quizzes/`, `gamification/`) — permission-boundary pytest
...
- [ ] [P] **Types** — add `is_locked: boolean` (and `lesson_count?: number`) to `Unit`
...
- [ ] **Instructor toggle** — `OutlineUnitCard` (`components/manage/OutlineUnitCard.tsx`):
...
- [ ] [P] **Student course detail** — `CourseDetailPage.tsx` unit cards (:450-498):
...
- [ ] [P] **Course map** — `CourseMapPath.tsx`: when `lock_reason === 'instructor'`,
