...
- [ ] Create `.claude/settings.json` with the **active** `permissions.deny` list — the prod-destructive set only:
...
- [ ] **Deliberate deviation, flagged:** `mcp__neon__run_sql` and
...
- [ ] Add the inert `_proposed` object (unknown top-level key, ignored by the
...
- [ ] Populate `_proposed.deny`: `Bash(git push --force:*)`, `Bash(git push -f:*)`,
...
- [ ] Populate `_proposed.ask`: `Bash(git push:*)`, `Bash(pip install:*)`,
...
- [ ] Populate `_proposed.allow` with the safe high-frequency set, so a later
...
- [ ] Add `_proposed._keys` noting `permissions.defaultMode`,
...
- [ ] Record in the file that Bash deny rules are a safety net, not a security
...
- [ ] Write a `PreToolUse` hook matching `Bash` that reads the tool-call JSON on
...
- [ ] Block: shell-wrapper evasion (`sh -c`, `bash -c`, `zsh -c`, `eval `),
      ...
      method).
- [ ] Fail **open** on a parse error or unexpected input and exit 0 — a
      guardrail that hard-fails every Bash call is worse than the gap it closes.
      Keep total runtime under ~100 ms.
- [ ] Register it in `.claude/settings.json` under `hooks.PreToolUse` with
...
- [ ] `backend.md` — `backend/**/*.py`. The always-on core, kept short: `/api/`
...
- [ ] `backend-views.md` [P] — `backend/**/views.py`, `backend/**/permissions.py`,
...
- [ ] `backend-serializers.md` [P] — `backend/**/serializers.py`. Nested content
...
- [ ] `backend-tests.md` [P] — `backend/**/test*.py`, `backend/conftest.py`.
...
- [ ] `backend-migrations.md` [P] — `backend/**/migrations/*.py`,
...
- [ ] `frontend.md` — `frontend/src/**/*.{ts,tsx}`. Always-on core: strict TS /
      ...
      `src/vite-env.d.ts`.
- [ ] `frontend-services.md` [P] — `frontend/src/services/**`. Everything goes
...
- [ ] `frontend-components.md` [P] — `frontend/src/components/**`,
      ...
      `@/contexts/useAuth`.
- [ ] `frontend-tests.md` [P] — `frontend/src/**/*.test.{ts,tsx}`,
...
- [ ] Rewrite `code-reviewer.md` — replace the inline instructor check with the
...
- [ ] Rewrite `adversarial-tester.md` — delete every probe for deadlines, grades,
...
- [ ] Rewrite `db-migration-checker.md` — remove `assignments`; make the
...
- [ ] Add `frontend-reviewer.md` [P] — a new agent covering axios calls that
...
- [ ] Keep the existing `model:` assignments (opus for `code-reviewer`, sonnet
      elsewhere); give `frontend-reviewer` `model: sonnet`.

### 5. Skills

- [ ] `verify-stack` — rewrite to full CI parity in CI's order:
...
- [ ] `finish-phase` — add `frontend-reviewer` to the step-3 parallel review;
...
- [ ] `handoff` — raise the length guidance from "under 60 lines" to "under 120,
      and prefer shorter"; the current 60 is being ignored rather than met.
- [ ] `start-phase` — update the folder map reference for the split rules dir.
      Otherwise unchanged.
- [ ] **Preserve frontmatter exactly** while rewriting any skill or agent:
...
- [ ] `.gitignore` — un-ignore `CLAUDE.md`; replace the blanket `.claude/` entry
...
- [ ] Note that committing `.claude/` brings it into scope of the `secret-scan`
...
- [ ] `CLAUDE.md` — work the ten numbered findings above. Concretely:
      ...
      - Keep the file under ~200 lines; it is loaded every session. If the
        ...
        letting it grow.
- [ ] `docs/CLAUDE-CODE-WORKFLOW.md` (stays private) — add `/finish-phase` to the
...
- [ ] `docs/runbooks/workflow-prompting-guide.txt` [P] — replace both
...
- [ ] `gh repo set-default Cesar6060/LMS`; rename remote `origin` →
      `old-origin` and `lms` → `origin`; re-point `main`'s upstream. Do not
      delete anything.
- [ ] Commit the pending phase-69 spec update and the untracked
...
- [ ] In `docs/specs/phase-69-robotics-2-course.md`, correct the
...
- [ ] Start a **fresh** Claude Code session in the repo and confirm no settings
...
- [ ] In that session, `gh pr merge --help` is **denied**, not prompted.
- [ ] `mcp__neon__delete_branch` is denied; a Neon read tool still works.
- [ ] `docker compose ps` still runs without a prompt (the existing local allow
      is unaffected).

**Hook**
- [ ] `echo '{"tool_name":"Bash","tool_input":{"command":"sh -c \"rm -rf /tmp/x\""}}' | .claude/hooks/guard-bash.sh; echo $?` → **2**.
- [ ] Same with `{"command":"git status"}` → **0**.
- [ ] Same with malformed input (`echo 'not json' | …`) → **0** (fails open).
- [ ] In a live session, a `sh -c` command is blocked with the hook's reason shown.
...
- [ ] Confirm no rule file exceeds ~80 lines.

**Agents**
- [ ] `grep -ri "assignments\|deadline\|submission\|graded" .claude/agents/` returns nothing.
- [ ] `grep -rn "request.user == course.instructor" .claude/agents/ .claude/rules/` returns nothing.
- [ ] Dispatch `frontend-reviewer` at `git diff main...HEAD` and confirm it
      returns a usable report rather than erroring on its tool list.

**Skills**
- [ ] `/verify-stack` runs all seven checks and ends in **PASS**. This is also the
      phase's own gate.
- [ ] `grep -n "cd backend && pytest" -r .claude/ CLAUDE.md` returns nothing.

**CLAUDE.md**
- [ ] Every command in the Commands block is pasted into a shell and runs:
...
- [ ] `grep -c "" CLAUDE.md` is under 200.
- [ ] `grep -n "push -u lms\|remote lms" CLAUDE.md` returns nothing — the remote
      rename makes that guidance wrong.
- [ ] It names all four skills: `grep -c "/start-phase\|/verify-stack\|/finish-phase\|/handoff" CLAUDE.md` ≥ 4.
- [ ] It states both deploy targets (Render backend **and** Cloudflare Pages frontend).
- [ ] Read it end to end once as a stranger would and confirm nothing secret or
      private slipped in — it is public after this phase.

**Git and visibility**
- [ ] `git check-ignore -v .claude/settings.local.json` matches; `git check-ignore -v .claude/rules/backend.md` does **not**.
- [ ] `git ls-files .claude | wc -l` is greater than 12; `git ls-files CLAUDE.md` returns the file.
- [ ] `git ls-files docs/PORTFOLIO.md docs/INTERVIEW_GUIDE.md docs/DEMO_SCRIPT.md docs/CLAUDE-CODE-WORKFLOW.md` returns **nothing** — the private docs stayed private.
- [ ] `git ls-files .claude/settings.local.json` returns nothing.
- [ ] `git ls-files .claude/scheduled_tasks.lock` returns nothing.
- [ ] `gh repo set-default --view` → `Cesar6060/LMS`; `git remote -v` shows `origin` → LMS.
- [ ] The `secret-scan` CI job is green on the PR — `.claude/` is newly in scope
      for both the history and working-tree gitleaks scans.
- [ ] Skill frontmatter survived: `grep -c "disable-model-invocation" .claude/skills/*/SKILL.md`
      is 1 for `handoff`, `start-phase` and `finish-phase`, and **0** for
      `verify-stack`.

**Docs**
- [ ] `grep -rn "stemquest-api.onrender.com" docs/runbooks/ CLAUDE.md .claude/`
      returns nothing — the phase-57 host is retired everywhere.
- [ ] `grep -rln "frontend-reviewer" .claude/agents/ .claude/skills/finish-phase/ docs/runbooks/`
      returns all three — the new agent is registered, wired into `/finish-phase`,
      and documented.

**Scope guard**
- [ ] `git diff main...HEAD --stat -- backend/ frontend/` is **empty**. No
